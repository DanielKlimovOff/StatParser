                ...
                std::unordered_map<std::string, std::function<void()>> switch_lang = {
                    {"cpp",  [this, file, path] () {
                        ...
                        try {
                            ...
                            if (!handle) {
                                std::cerr << "\t\tcancel: cant open module\n";
                                return;
                            }
                            ...
                            if ((error = dlerror()) != NULL) {
                                std::cerr << "\t\tcancel: cant load start (" << error << ")\n";
                                return;
                            }
                    ...
                    {"py", [this, file, path]() {
                        ...
                        try {
                            ...
                            if (handle != NULL) {
                                ...
                                /*=========================INFO CHECK=========================*/
                                /*=========================START=========================*/
                                routes[path] = [this, file, error](std::string *s, responser::response *rep, requester::request_data req, std::unordered_map<std::string, json> access_info, void* funcs ) {
                                    Py_Initialize();
                                    try {
                                        auto handshake = [](int sig) -> void {
                                            throw std::runtime_error("something wrong with api");
                                        ...
                                        for(auto t : access_info[req.request_info.path]["import"]) {
                                            PyRun_SimpleString( ("import " + t.get<std::string>()).c_str() );
                                        }
                                        ...
                                        if (handle != NULL) {
                                            ...
                                            if (start != NULL) {
                                                if (PyCallable_Check(start)) {
                                                    ...
                                                    PyDict_SetItemString(d, "access_info", PyUnicode_FromString(access_info[req.request_info.path].dump().c_str()));
                                                    ...
                                                    if(pResult) {
                                                        if (PyDict_Check(pResult)) {
                                                            ...
                                                            *s = (std::string) PyUnicode_AsUTF8(
                                                                    PyDict_GetItemString(pResult,
                                                                                         ((std::string) ("body_text")).c_str()));
                                                            rep->header_body += (std::string) PyUnicode_AsUTF8(
                                                                    PyDict_GetItemString(pResult, "header_body"));
                                                        }
                                                    }
                                                    else {
                                                        error();
                            ...
                            std::cerr << "\t\tError run: " << err.what() << std::endl;
                        }
                    }},
                    {"lua", [this, file, path]() {
                        ...
                        //if(luaL_dofile(L, file.path().string() + "main.lua"))
